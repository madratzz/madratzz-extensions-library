...
namespace MadRatz.Extensions
{
	public static class MonoBehaviourExtension
	{
		...
		public static Coroutine ExecuteCountUntil(this MonoBehaviour monoBehaviour, Action action, int count,
		                                          Func<bool> condition)
		{
		...
		public static Coroutine ExecuteIntervalUntil(this MonoBehaviour monoBehaviour, Action action, float interval,
		                                             int count, Func<bool> condition, bool timeScale = true)
		{
			return monoBehaviour.StartCoroutine(ExecuteIntervalUntilCoroutine(action, count, interval, condition,
				timeScale));
		}

		private static IEnumerator ExecuteIntervalUntilCoroutine(Action action, int count, float interval,
		                                                         Func<bool> condition, bool timeScale)
		{
			var counter = 0;
		...
		public static Coroutine ExecuteIntervalUntil(this MonoBehaviour monoBehaviour, Action action, float interval,
		                                             Func<bool> condition, bool timeScale = true)
		{
			return monoBehaviour.StartCoroutine(ExecuteIntervalUntilCoroutine(action, interval, condition, timeScale));
		}

		private static IEnumerator ExecuteIntervalUntilCoroutine(Action action, float interval, Func<bool> condition,
		                                                         bool timeScale)
		{
			while (condition != null && !condition())
			{
		...
		public static Coroutine ExecuteIfUntil(this MonoBehaviour monoBehaviour, Action action,
		                                       Func<bool> executeCondition, Func<bool> finishCondition)
		{
			return monoBehaviour.StartCoroutine(ExecuteIfUntilCoroutine(action, executeCondition, finishCondition));
		}

		private static IEnumerator ExecuteIfUntilCoroutine(Action action, Func<bool> executeCondition,
		...
		public static Coroutine ExecuteDelay(this MonoBehaviour monoBehaviour, Action action, float seconds,
		                                     bool timeScale = true)
		{
		...
		public static Coroutine ExecuteInterval(this MonoBehaviour monoBehaviour, Action action, float interval,
		                                        bool timeScale = true)
		{
		...
		public static Coroutine ExecuteInterval(this MonoBehaviour monoBehaviour, Action action, float interval,
		                                        float duration, bool timeScale = true)
		{
			return monoBehaviour.StartCoroutine(ExecuteIntervalCoroutine(action, interval, duration, timeScale));
		}

		private static IEnumerator ExecuteIntervalCoroutine(Action action, float interval, float duration,
		                                                    bool timeScale)
		{
			var timer = 0f;
			var executeTimer = 0f;
		...
		public static Coroutine ExecuteIntervalCount(this MonoBehaviour monoBehaviour, Action action, int count,
		                                             float interval, bool timeScale = true)
		{
			return monoBehaviour.StartCoroutine(ExecuteIntervalCoroutine(action, count, interval, timeScale));
		}

		private static IEnumerator ExecuteIntervalCountCoroutine(Action action, int count, float interval,
		                                                         bool timeScale)
		{
			for (var i = 0; i < count; i++)
			{
