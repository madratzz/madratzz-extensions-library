...
namespace MadRatz.Extensions
{
	public static partial class Vector2Extension
	{
		...
		public static Vector2 GetPointOn2DCircle(Vector3 center, float radius, float angle)
		{
			var pos = new Vector3
			{
				y = center.z + (radius * Mathf.Cos(angle / (180f / Mathf.PI))),
				z = center.y,
				x = center.x + (radius * Mathf.Sin(angle / (180f / Mathf.PI)))
			};
			return pos;
		}
