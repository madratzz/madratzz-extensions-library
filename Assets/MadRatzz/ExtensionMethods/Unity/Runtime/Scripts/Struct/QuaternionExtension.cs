...
namespace MadRatz.Extensions
{
	public static class QuaternionExtension
	{
		...
		public static void Deconstruct(in this Quaternion quaternion, out float x, out float y, out float z,
		                               out float w)
		{
			x = quaternion.x;
			y = quaternion.y;
		...
		public static Quaternion With(in this Quaternion quaternion, float? x = null, float? y = null, float? z = null,
		                              float? w = null)
		{
		...
		public static Quaternion WithEuler(this Quaternion quaternion, float? x = null, float? y = null,
		                                   float? z = null)
		{
		...
		public static Quaternion Pow(this Quaternion quaternion, float power)
		{
			...
			var vectorBit = new Quaternion(nHat.x, nHat.y, nHat.z, 0)
			               .ScalarMultiply(power * Mathf.Acos(quaternion.w / inputMagnitude)).Exp();
		...
		{
			var result = Mathf.Sqrt(quaternion.x * quaternion.x + quaternion.y * quaternion.y +
		...
		{
			var result = new Quaternion(quaternion.x * scalar, quaternion.y * scalar, quaternion.z * scalar,
				quaternion.w * scalar);
			return result;
		}
